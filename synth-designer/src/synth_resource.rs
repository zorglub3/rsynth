...
impl SynthResource {
    ...
    pub fn add_data_buffers(&mut self, module_name: &str, data_buffers: Vec<Vec<f32>>) {
        self.data_buffers
            .insert(module_name.to_string(), data_buffers);
    }

    pub fn add_code_buffers(&mut self, module_name: &str, code_buffers: Vec<Vec<Instr>>) {
        self.code_buffers
            .insert(module_name.to_string(), code_buffers);
    }
    ...
    */
    ...
    pub fn get_code_buffer(
        &self,
        module_name: &str,
        index: usize,
    ) -> Result<&[Instr], ModuleError> {
